...
export default class CityListManager {
  /**
     * 添加该城市到缓存中去
     */
  static addCity(cityId, country, district, province, temp,
    max, min, weatherDesc, weatherType, isLocation, complete) {
    const cityItem = this.generateCityItem(cityId, country, district, province, temp,
      max, min, weatherDesc, weatherType, isLocation);
    this.findCity(cityItem, (hasCity, cityList) => {
  ...
  /**
     * 更新城市列表
     */
  static updateCity(cityId, country, district, province, temp,
    max, min, weatherDesc, weatherType, isLocation) {
    let updateCity = this.generateCityItem(cityId, country, district, province, temp,
      max, min, weatherDesc, weatherType, isLocation);
  ...
  static findCity(item, callBack) {
    this.getCityList((cityList) => {
      ...
      if (cityList && cityList.length > 0) {
        for (let i = 0; i < cityList.length; i++) {
          const cityItem = cityList[i];
          if (item.district == cityItem.district && item.country == cityItem.country) {
            hasCity = true;
            break;
          }
        }
      ...
      if (cityList && cityList.length > 0) {
        ...
        cityList.map((cityItem) => {
          ...
          for (let i = 0; i < deleteCityList.length; i++) {
            ...
            if ((cityItem.district === deleteCity.district)) {
              // 如果需要删除
              needDelete = true;
              break;
            }
  ...
  /**
     ...
     * @returns {{cityId: *, country: *, district: *, province: *, max: *, min: *, isLocation: *}}
     */
  static generateCityItem(cityId, country, district, province, temp,
    max, min, weatherDesc, weatherType, isLocation) {
    return {
      cityId: cityId,
      country: country,
